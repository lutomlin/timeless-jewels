        ...
        <div class="flex flex-col overflow-auto min-h-[200px] mb-2">
          <VirtualList
            ...
            <div slot="item" let:index let:style {style}>
              <SearchResult set={searchResults.grouped[k][index]} {highlight} {jewel} {conqueror} {platform} {league} {anyConqueror} />
            </div>
          </VirtualList>
        </div>
      {/if}
...
{:else}
  <div class="mt-4 flex flex-col overflow-auto">
    <VirtualList
      ...
      <div slot="item" let:index let:style {style}>
        <SearchResult set={searchResults.raw[index]} {highlight} {jewel} {conqueror} {platform} {league} {anyConqueror} />
      </div>
    </VirtualList>
  </div>
{/if}
